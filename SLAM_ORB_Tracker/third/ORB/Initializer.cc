...
bool Initializer::Initialize(shared_ptr<FrameState> CurrentFrame, const vector<int> &vMatches12, cv::Mat &R21, cv::Mat &t21,
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    // Fill structures with current keypoints and matches with reference frame
    // Reference Frame: 1, Current Frame: 2
    mvKeys2 = CurrentFrame->mvKeyPoint;
    ...
    mvSets = vector< std::vector<size_t> >(mMaxIterations,std::vector<size_t>(8,0));
    ...
    //else //if(pF_HF>0.6)
        return ReconstructF(vbMatchesInliersF,F,mK,R21,t21,vP3D,vbTriangulated,1.0f,50);
